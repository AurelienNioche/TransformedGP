...
transformed parameters{
  ...
  K = cov_exp_quad(to_array_1d(x_eval), sigmasq, l) + diag_matrix(rep_vector(jitter, N_eval));
...
model {
  ...
  K_inv = inverse_spd(K);
  K0 = cov_exp_quad(to_array_1d(x0), to_array_1d(x_eval), sigmasq, l);
  K1 = cov_exp_quad(to_array_1d(x1), to_array_1d(x_eval), sigmasq, l);
