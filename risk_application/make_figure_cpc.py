...
from accuracy_comparison.accuracy_comparison \
    import accuracy_comparison_cpc, accuracy_comparison_single


def individual_plots(df_dm, fig_name_ext):
    ...
    for s in tqdm(df_dm.subject):
        is_s = df_dm.subject == s
        dm = df_dm.loc[is_s, "dm"].item()
        fig = plot_single(dm=dm, s=s)
        figs.append(fig)

    fig_name = f"fig/risk_cpc_individual{fig_name_ext}.pdf"
