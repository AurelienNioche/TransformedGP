...
def train_gp(data, epochs=100, noise_init_value=50., init_lr=0.1):
    ...
    optimizer = torch.optim.Adam(gp.parameters(), lr=init_lr)  # Includes GaussianLikelihood parameters
...
def train_discrepancy_model(data, m, theta, epochs=100, init_lr=0.1, noise_init_value=50):
...
model, likelihood, hist_loss = train_discrepancy_model(data, m, theta, epochs=1000)
